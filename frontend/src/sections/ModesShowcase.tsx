...
const TAB_SPRING = { type: 'spring' as const, stiffness: 420, damping: 34, mass: 0.8 };
...
function mergeIntervals(intervals) {
  ...
  },
  {
    ...
    features: ['Timed locked sessions', 'Integrity timeline (not verdict)', 'Academic progress tracking', 'Transparent evaluation'],
...
const ModesShowcase = () => {
  ...
  return (
    <section id="modes" style={{ padding: '100px 32px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        >
          ...
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: 'rgba(255,255,255,0.36)', lineHeight: 1.75, maxWidth: '520px', margin: '0 auto' }}>
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.04)', borderRadius: '9px' }}
                    transition={TAB_SPRING}
                  />
                )}
                <span style={{ position: 'relative', zIndex: 1, fontSize: '14px' }}>{mode.icon}</span>
                ...
                <span style={{
                  ...
                  color: isActive ? 'rgba(255,255,255,0.52)' : 'rgba(255,255,255,0.22)',
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            style={{
              ...
              background: 'linear-gradient(160deg, rgba(255,255,255,0.024) 0%, rgba(255,255,255,0.010) 100%)',
          ...
          >
            {/* Left */}
            <div>
              <div style={{ fontSize: '22px', color: active.accentColor, marginBottom: '16px', fontFamily: 'JetBrains Mono, monospace' }}>
                {active.icon}
              </div>
              <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '32px', letterSpacing: '-0.028em', color: 'rgba(255,255,255,0.90)', margin: '0 0 12px', lineHeight: 1.1 }}>
                {active.title}
              </h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '11px' }}>
                {active.features.map((f, i) => (
                  <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <span style={{ color: active.accentColor, fontSize: '9px', flexShrink: 0, opacity: 0.8 }}>▪</span>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', color: 'rgba(255,255,255,0.48)' }}>{f}</span>
                  </div>
                ))}
              </div>
              ...
              {activeId === 'pair' && (
                <motion.button
                  ...
                  onMouseEnter={e => {
                    ...
                    e.currentTarget.style.borderColor = 'rgba(120,170,240,0.45)';
                    e.currentTarget.style.color = 'rgba(120,170,240,1)';
                  }}
                  onMouseLeave={e => {
                    e.currentTarget.style.background = 'rgba(120,170,240,0.10)';
                    e.currentTarget.style.borderColor = 'rgba(120,170,240,0.28)';
            ...
            {/* Right - code preview */}
            <div style={{ background: '#0c1018', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', overflow: 'hidden', position: 'relative' }} className="hp-code-preview">
              <div style={{ background: 'rgba(255,255,255,0.025)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', gap: '5px' }}>
                  {['#ff5f57', '#febc2e', '#28c840'].map(c => (
                    <div key={c} style={{ width: '9px', height: '9px', borderRadius: '50%', background: c, opacity: 0.7 }} />
                  ))}
                </div>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: 'rgba(255,255,255,0.22)', letterSpacing: '0.08em' }}>
                  PREVIEW · {active.id.toUpperCase()}
                </span>
              </div>
              <pre style={{
                padding: '24px', margin: 0,
                fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', lineHeight: 1.85,
                color: 'rgba(255,255,255,0.48)', whiteSpace: 'pre-wrap', wordBreak: 'break-word',
              }}>
                <span style={{ color: active.accentColor }}>{active.codeSnippet.split('\n').slice(0, 3).join('\n')}</span>
                {'\n'}
                <span style={{ color: 'rgba(255,255,255,0.55)' }}>{active.codeSnippet.split('\n').slice(4).join('\n')}</span>
              </pre>
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '48px', background: 'linear-gradient(transparent, #0c1018)', pointerEvents: 'none' }} />
            </div>
          </motion.div>
        </AnimatePresence>
