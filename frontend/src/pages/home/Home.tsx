...
interface LocationState { toastMessage?: string; }
...
const Home = () => {
  ...
  const generateRoomId = () => {
    const id = v4(); setRoomId(id);
    navigator.clipboard.writeText(id).then(() => toast.success('Room ID copied to clipboard')).catch(() => {});
  };

  const createRoom = (e: FormEvent) => {
    e.preventDefault();
    if (!username || username.trim().length < 2) { toast.error('Please enter your name (at least 2 characters)'); return; }
    const id = v4(); const u = username.trim();
    navigator.clipboard.writeText(id).then(() => toast.success('Room ID created & copied')).catch(() => {});
    navigate(`/editor/${id}`, { state: { username: u } });
  };

  const joinRoom = async (): Promise<void> => {
    if (!roomId || !username) { toast.error('Room ID & username are required'); return; }
    const r = roomId.trim(); const u = username.trim();
    if (!isValidUUID(r)) { toast.error('Invalid Room ID format'); return; }
    if (u.length < 2) { toast.error('Username must be at least 2 characters'); return; }
    if (u.length > 30) { toast.error('Username cannot exceed 30 characters'); return; }
    if (!/^[a-zA-Z0-9_\s]+$/.test(u)) { toast.error('Username can only contain letters, numbers, spaces, and underscores'); return; }
    if (isJoining) return;
    setIsJoining(true);
    try {
      ...
      if (!data.exists) { toast.error('Room does not exist. Check the ID or create a new room.'); setIsJoining(false); return; }
      navigate(`/editor/${r}`, { state: { username: u } });
    } catch { toast.error('Failed to validate room. Please try again.'); setIsJoining(false); }
  };

  const handleInputEnter = (e: KeyboardEvent) => { if (e.code === 'Enter') void joinRoom(); };
