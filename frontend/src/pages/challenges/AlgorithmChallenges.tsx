...
export default function AlgorithmChallenges() {
  ...
  const getStatus = useCallback(
    (problem: DSAProblem, index: number): 'completed' | 'unlocked' | 'locked' => {
  ...
  const completedCount = config.problems.filter((p) => completed.has(p.slug)).length;

  return (
    <div className="algo-challenges">
      {/* Header */}
      <header className="algo-challenges__header">
        <button className="algo-challenges__back" onClick={() => navigate('/features')}>
          <ArrowLeft size={18} />
          <span>Back</span>
        </button>
        ...
        <div className="algo-challenges__progress-pill">
          ...
          <span>
            {completed.size} / {easyProblems.length + mediumProblems.length + hardProblems.length}{' '}
            solved
          </span>
        </div>
      </header>
        ...
        {config.problems.map((problem, idx) => {
          ...
          return (
            <motion.button
              ...
              onClick={() => status !== 'locked' && navigate(`/practice/${problem.slug}`)}
            ...
            >
              ...
              <div className="level-card__body">
                <p className="level-card__topic">{problem.topic.replace(/-/g, ' ')}</p>
