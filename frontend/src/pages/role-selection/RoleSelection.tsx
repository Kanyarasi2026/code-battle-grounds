	...
	<>
		...
		<motion.div
			...
			style={{
				...
				borderRadius: '50%',
				background:
					'radial-gradient(circle, rgba(255,255,255,0.014) 0%, transparent 70%)',
				filter: 'blur(70px)',
				pointerEvents: 'none',
		...
		<motion.div
			...
			animate={{ y: [0, -18, 0], x: [0, 12, 0], opacity: [0.4, 0.75, 0.4] }}
			transition={{
				duration: 20,
				repeat: Infinity,
				ease: 'easeInOut',
		...
		<motion.div
			...
			animate={{ y: [0, 16, 0], x: [0, -10, 0], opacity: [0.35, 0.7, 0.35] }}
			transition={{
				duration: 22,
				repeat: Infinity,
				ease: 'easeInOut',
...
const roles: RoleCard[] = [
	{
		...
		description:
			'Sharpen your coding skills with algorithm challenges, mock interviews, and collaborative problem-solving.',
		features: [
	...
	},
	{
		id: 'academic',
		icon: Construction,
		title: 'COMING SOON: Academic Features',
		subtitle: 'COMING SOON: Academic Environment',
		description:
			'Practice, learn, and assess in a structured academic setting with AI-powered coaching and transparent evaluation tools.',
		features: [
...
const academicRoles: AcademicRoleCard[] = [
	{
		...
		description:
			'Create and manage assessments, track student progress, and maintain academic integrity.',
		features: [
	...
	},
	{
		...
		description:
			'Take assessments, practice coding problems, and track your progress.',
		features: [
...
const RoleSelection = () => {
	const CONSTRUCTION_POPUP_TIMEOUT_MS = 5000;
	const navigate = useNavigate();
	const location = useLocation();
	const { user, requestRole } = useAuth();
	const [selectedRole, setSelectedRole] = useState<Role | null>(null);
	const [selectedAcademicRole, setSelectedAcademicRole] =
		useState<AcademicRole | null>(null);
	const [isAnimating, setIsAnimating] = useState(false);
	const [hoveredCard, setHoveredCard] = useState<string | null>(null);
	const [showConstructionPopup, setShowConstructionPopup] = useState(false);
	const constructionPopupTimerRef = useRef<number | null>(null);
	...
	const clearConstructionTimer = () => {
		if (constructionPopupTimerRef.current) {
			window.clearTimeout(constructionPopupTimerRef.current);
			constructionPopupTimerRef.current = null;
		}
	};
	...
	const openConstructionPopup = () => {
		...
		setIsAnimating(false);
		clearConstructionTimer();
		constructionPopupTimerRef.current = window.setTimeout(() => {
			setShowConstructionPopup(false);
			setSelectedRole(null);
			setIsAnimating(false);
		}, CONSTRUCTION_POPUP_TIMEOUT_MS);
	};

	useEffect(() => () => clearConstructionTimer(), []);
		...
		return (
			<div className="role-selection">
				...
				>
					...
					>
						<p className="role-selection__welcome">
							Welcome,{' '}
					...
					<motion.div
						className="role-selection__cards"
						variants={containerVariants}
					>
						{academicRoles.map((role) => {
							...
							const isOtherSelected =
								selectedAcademicRole && selectedAcademicRole !== role.id;
		...
		<div className="role-selection">
			...
			>
				<motion.div className="role-selection__header" variants={itemVariants}>
					...
					<p className="role-selection__subtitle">
						Select your role to customize your experience and access relevant
						features
					</p>
				</motion.div>
			...
			{showConstructionPopup && (
				<motion.div
					className="construction-popup__backdrop"
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					exit={{ opacity: 0 }}
				>
					<motion.div
						...
						role="dialog"
						aria-modal="true"
						aria-label="Academic section under construction"
					...
					>
						<button
							type="button"
							className="construction-popup__close"
						...
						<div className="construction-popup__tape">UNDER CONSTRUCTION</div>
						<div className="construction-popup__icon-wrap">
							<motion.div
								className="construction-popup__icon"
								animate={{ rotate: [0, -9, 9, -6, 0] }}
								transition={{
									duration: 1.8,
									repeat: Infinity,
									ease: 'easeInOut',
							...
							>
								<Construction size={38} strokeWidth={2} />
							</motion.div>
						</div>

						<h3 className="construction-popup__title">
							Academic Zone In Progress
						</h3>
						<p className="construction-popup__text">
							We are building the academic experience right now. Please check
							back soon while we finish the scaffolding.
						</p>
						<p className="construction-popup__hint"></p>
					</motion.div>
				</motion.div>
			)}
		</div>
